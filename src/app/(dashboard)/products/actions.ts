...
/**
 * 제품 복원 (is_active = true)
 */
export async function restoreProduct(id: string): Promise<ActionResult<ProductRow>> {
  try {
    const supabase = await createClient();
    const {
      data: { user },
    ...
    if (error) {
      return { error: `제품 복원 실패: ${error.message}` };
    }
    ...
    return {
      error: `제품 복원 중 오류 발생: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
}
